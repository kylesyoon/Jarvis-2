...
class JARMotionController: NSObject {
    ...
    func analyzeMotion(motion: CMDeviceMotion!) {
        ...
        if let previousAttitude = self.previousAttitude {
            ...
            let roll = self.getDegreesFromRadians(previousAttitude.roll)
            let pitch = self.getDegreesFromRadians(previousAttitude.pitch)
            let yaw = self.getDegreesFromRadians(previousAttitude.yaw)
